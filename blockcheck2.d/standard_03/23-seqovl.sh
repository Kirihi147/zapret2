...
pktws_seqovl_tests_tls()
{
	...
	[ "$NOTEST_PADENCAP_HTTPS" = 1 ] || { pktws_curl_test_update $testf $domain ${SEQOVL_PATTERN_HTTPS:+--blob=$pat:@"$SEQOVL_PATTERN_HTTPS" }$pre $PAYLOAD $padencap_mod --lua-desync=tcpseg:pos=0,-1:seqovl=#patmod:seqovl_pattern=patmod --lua-desync=drop && ok=1; }
	ok_any=$ok
	...
	for split in 10 10,sniext+1 10,sniext+4 10,midsld; do
		...
		[ "$NOTEST_PADENCAP_HTTPS" = 1 ] || { pktws_curl_test_update $testf $domain ${SEQOVL_PATTERN_HTTPS:+--blob=$pat:@"$SEQOVL_PATTERN_HTTPS" }$pre $PAYLOAD $padencap_mod --lua-desync=multisplit:pos=$split:seqovl=#patmod:seqovl_pattern=patmod && ok=1; }
		[ "$ok" = 1 -a "$SCANLEVEL" != force ] && break
	done
