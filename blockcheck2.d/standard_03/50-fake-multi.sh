		...
		for fooling in $FOOLINGS_TCP; do
			for split in $splits; do
				for ff in $fake 0x00000000; do
					...
					[ "$NOTEST_OUTRANGE_HTTPS" != 1 -o "$PAYLOAD" != "--payload=tls_client_hello" ] && contains "$fooling" tcp_md5 && pktws_curl_test_update $testf $domain ${FAKE_HTTP:+--blob=fake_http:@"$FAKE_HTTP" }$PAYLOAD --lua-desync=fake:blob=$ff:$fooling:repeats=$FAKE_REPEATS --lua-desync=$splitf:pos=$split --payload=empty "--out-range=<s1" --lua-desync=send:$TCP_MD5 && ok=1
				done
			done
		done
		for ttl in $attls; do
...
pktws_fake_https_vary_()
{
	...
	[ "$NOTEST_BLANK_BLOB_HTTPS" = 1 ] || { pktws_curl_test_update $testf $domain $pre $PAYLOAD --lua-desync=fake:blob=0x00000000:$fooling:repeats=$FAKE_REPEATS --lua-desync=$splitf:pos=$split $post && ok_any=1; }
	[ "$NOTEST_BLANK_BLOB_HTTPS" = 1 ] || { pktws_curl_test_update $testf $domain ${FAKE_HTTPS:+--blob=$fake:@"$FAKE_HTTPS" }$pre $PAYLOAD --lua-desync=fake:blob=0x00000000:$fooling:repeats=$FAKE_REPEATS --lua-desync=fake:blob=$fake:$fooling:tls_mod=$tls_mod:repeats=$FAKE_REPEATS --lua-desync=$splitf:pos=$split $post && ok_any=1; }
	pktws_curl_test_update $testf $domain ${FAKE_HTTPS:+--blob=$fake:@"$FAKE_HTTPS" }$pre $PAYLOAD --lua-desync=fake:blob=$fake:$fooling:tls_mod=$tls_mod:repeats=$FAKE_REPEATS --lua-desync=$splitf:pos=$split $post && ok_any=1
	...
	[ "$NOTEST_PADENCAP_HTTPS" = 1 ] || { pktws_curl_test_update $testf $domain ${FAKE_HTTPS:+--blob=$fake:@"$FAKE_HTTPS" }$pre $PAYLOAD --lua-desync=fake:blob=$fake:$fooling:tls_mod=$tls_mod,padencap:repeats=$FAKE_REPEATS --lua-desync=$splitf:pos=$split $post && ok_any=1; }
	[ "$ok_any" = 1 ] && ok=1
}
